...
accent_lt  = "#FDECEA"          # light red tint
accent_mid = "#E74C3C"          # brighter red for hover accents
muted_text = "#6B7280"
body_text  = "#1A1A1A"
...
.car-name {{ font-weight: 700; font-size: 15px; color: {body_text}; }}
.car-brand {{ font-size: 12px; color: {muted_text}; margin-bottom: 6px; }}
.car-desc {{ font-size: 13px; color: #4B5563; line-height: 1.5; }}
.badge {{
    display: inline-block;
    background: {accent_lt};
...
with tab1:
    ...
    def score_card(col, model, label, value, highlight=False):
        border = f"border: 1.5px solid {accent}; background: {accent_lt};" if highlight else ""
        col.markdown(f"""
...
<div class='card' style='text-align:center; padding:40px 20px;'>
  ...
  <div class='card-small' style='margin-top:10px;'>Fill in car details and hit<br>
    <strong style='color:{accent};'>Get Prediction</strong>
  </div>
</div>""", unsafe_allow_html=True)
...
st.markdown("<hr>", unsafe_allow_html=True)
st.markdown(f"<div style='text-align:center; color:{muted_text}; font-size:12px;'>SmartCar AI &nbsp;·&nbsp; CarDekho Dataset &nbsp;·&nbsp; Linear Regression + Random Forest</div>",
    unsafe_allow_html=True)
